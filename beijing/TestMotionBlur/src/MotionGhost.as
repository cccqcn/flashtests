...
package  {
	 ...
	 */
	public class MotionGhost extends TweenPlugin {
		/** @private **/
		...
		private var cachedTime:Number = 0;
		
		/** @private **/
		public function MotionGhost() {
			super();
		}
		...
		override public function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			...
			if ("x" in _tween._propLookup && "y" in _tween._propLookup && !_tween._propLookup.x.isPlugin && !_tween._propLookup.y.isPlugin) { //if the tweens are plugins, like bezier or bezierThrough for example, we cannot assume the angle between the current _x/_y and the destination ones is what it should start at!
				_angle = 180 - Math.atan2(_tween._propLookup.y.change, _tween._propLookup.x.change) * _RAD2DEG;
		...
		override public function setRatio(n:Number):void
		{
			...
			_time = getTimer();
			var time:Number = (cachedTime - _time)/1000;
			if (time < 0) {
