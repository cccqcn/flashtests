package
{
	...
	{ 
		...
		{ 
			...
			this.mapModel.map = this.map;//create map data
		...
		{ 
			...
			fromp.toString();
			ui.startpos.text = fromp.toString();
			ui.endpos.text = top.toString();
			Path.instance.modifyflag = ui.modifybtn.selected;
			if(ui.abtn.selected)
			{
				this.path = this.aStar.find(playPoint.clone(), new Point(int(screen.mouseX),int(screen.mouseY)));//get route
			}
			else
			{
				this.path = Path.instance.find(fromp, top);//this.aStar.find(playPoint.clone(), new Point(int(screen.mouseX),int(screen.mouseY)));//get route
				convertPath();
			}
			var s:String = "";
			if (!this.path || path.length == 0 || path.length == 1) 
				s = "No route: "+ (getTimer() - t)+"ms"; 
			else 
				s = "Time: "+ (getTimer() - t)+"ms"; 
			s += " (" + new Date().toString() + ")";
			if(this.path)
			{
				s += "\n" + this.path.toString();
